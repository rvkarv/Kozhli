...
class NakshatraCalculator {
  ...
  static MoonPosition computeCurrent(DateTime utc) {
    assert(
      utc.isUtc,
      'Pass a true UTC DateTime, not a shifted/local one.',
    );
    ...
    // SUN
    //
    // Used for lunar elongation / waxing-waning calculation.
    // ---------------------------------------------------------------

    final sunMeanAnomaly = _mod360(
      357.52911 +
          t *
              (35999.05029 -
                  0.0001537 * t),
    ...
    final sunGeometricMeanLongitude = _mod360(
      280.46646 +
          t *
              (36000.76983 +
                  0.0003032 * t),
    ...
    final sunEquationOfCenter =
        math.sin(_rad(sunMeanAnomaly)) *
                (1.914602 -
                    t *
                        (0.004817 +
                            0.000014 * t)) +
            math.sin(
                  _rad(2.0 * sunMeanAnomaly),
                ) *
                (0.019993 -
                    0.000101 * t) +
            math.sin(
                  _rad(3.0 * sunMeanAnomaly),
                ) *
                0.000289;

    final sunTrueLongitude =
        sunGeometricMeanLongitude +
            sunEquationOfCenter;

    final sunApparentLongitude =
        sunTrueLongitude -
            0.00569 -
            0.00478 *
                math.sin(
    ...
    // ---------------------------------------------------------------
    // PRINCIPAL LUNAR LONGITUDE PERIODIC TERMS
    // ---------------------------------------------------------------
    ...
    for (final term in _moonTerms) {
      ...
      final eFactor =
          math.pow(e, m.abs()).toDouble();

      final argument =
          d * moonMeanElongation +
          m * sunMeanAnomaly +
          mp * moonMeanAnomaly +
          f * moonArgumentOfLatitude;
    ...
    // ---------------------------------------------------------------
    // ADDITIONAL LUNAR LONGITUDE CORRECTIONS
    //
    // Meeus Table 47.A additional terms.
    // ---------------------------------------------------------------
    ...
    final moonTropicalLongitude = _mod360(
      moonMeanLongitude +
          sigmaL / 1000000.0,
    );
    ...
    // LAHIRI AYANAMSA
    //
    // The existing application uses a linear Lahiri approximation.
    // Keep this deterministic and continuous across the year.
    //
    // The value is approximately 24°13' for 2026.
    // ---------------------------------------------------------------
    ...
    final moonSiderealLongitude = _mod360(
    ...
    final nakshatraStart =
        (nakshatraIndex - 1) *
            _nakshatraSize;

    final withinNakshatra =
        moonSiderealLongitude -
            nakshatraStart;
  ...
  static double _lahiriAyanamsa(
    ...
    // Lahiri sidereal offset used by the
    // existing workbook/application model.
    //
    // 291 is the reference epoch year
    // used by the workbook's linear formula.
    const arcSecondsPerYear =
        50.2388475;
