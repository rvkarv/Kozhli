...
DateTime _dt(int day, int hour, int minute, int second, int microsecond) {
  final milliseconds = microsecond ~/ 1000;
  final remainingMicroseconds = microsecond % 1000;
  return DateTime(
    2026,
    8,
    day,
...
void main() {
  test('Friday waxing day KOZHLI windows match Excel-calculated timings', () {
    final sunrise = _dt(14, 5, 46, 5, 443000);
    final sunset = _dt(14, 18, 29, 9, 913000);
    ...
    // Master Excel Main!F7 = (day samam - 02:24) / 5.
  ...
  test('Friday waxing night KOZHLI windows match Excel sunset-to-next-sunrise timings', () {
    ...
    // Master Excel Main!F9 = (02:24 - night samam) / 5.
    ...
    _expectNear(windows[0].start, _dt(14, 20, 44, 35, 640000));
    _expectNear(windows[0].end, _dt(14, 21, 12, 52, 785000));

    expect(windows[1].bird, Pakshi.kozhi);
    expect(windows[1].activity, Thozhil.oon);
    expect(windows[1].percent, 75);
    _expectNear(windows[1].start, _dt(15, 1, 15, 27, 94000));
    _expectNear(windows[1].end, _dt(15, 1, 25, 44, 239000));

    expect(windows[2].bird, Pakshi.kozhi);
    expect(windows[2].activity, Thozhil.arasu);
    expect(windows[2].percent, 100);
    _expectNear(windows[2].start, _dt(15, 3, 30, 52, 821000));
    _expectNear(windows[2].end, _dt(15, 4, 17, 9, 966000));
  });
}
